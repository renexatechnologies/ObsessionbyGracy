...
export const HomePageDesktop: React.FC<HomePageDesktopProps> = () => {
  ...
  return (
    <div className="text-on-background min-h-screen flex flex-col bg-[#8C9567]">
      <TopNavBar />
      ...
      <section className="relative w-full h-[100vh] min-h-[600px] flex items-center justify-center overflow-hidden">
        ...
        {/* Hero content */}
        <div className="relative z-10 text-center px-6 max-w-3xl mx-auto animate-[fadeInUp_1s_ease-out_both]">
          <p className="font-label-md text-label-md text-white/70 uppercase tracking-[0.3em] mb-4">
            Est. 1924 — Heritage Bakery
          </p>
          <h1 className="font-headline-xl text-[40px] md:text-[64px] leading-[1.05] text-white mb-6 drop-shadow-lg">
            Welcome to <br className="hidden md:block"/>Gracy's Kitchen
          </h1>
          <p className="font-body-lg text-[16px] md:text-body-lg max-w-xl mx-auto mb-10 text-white/85 leading-relaxed">
          ...
          <Link
            to="/catalog"
            className="inline-block bg-white text-[#30111C] rounded-full px-10 py-4 font-button text-[15px] hover:bg-white/90 transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-0.5"
          >
            EXPLORE COLLECTION
          </Link>
        </div>
      ...
      {/* Value Prop Section */}
      <section className="pt-20 md:pt-28 pb-10 md:pb-14 px-gutter bg-[#8C9567]">
        <div
        ...
        >
          <p className="text-center font-label-md text-label-md text-white/70 uppercase tracking-[0.25em] mb-3">Why Gracy's</p>
          <h2 className="text-center font-headline-md text-headline-md text-white mb-12 md:mb-16">Crafted with Purpose</h2>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 md:gap-6">
            {valueProps.map((prop, index) => (
              <div
                key={index}
                className="flex flex-col items-center text-center p-5 md:p-6 rounded-2xl bg-[#FAF7F2] shadow-lg hover:-translate-y-2 hover:shadow-xl transition-all duration-500 group"
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full overflow-hidden mb-4 ring-2 ring-[#8C9567]/20 group-hover:ring-[#8C9567]/40 transition-all duration-500">
                ...
                <p className="font-label-md text-label-md text-[#3a3a2e] leading-snug">{prop.title}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#8C9567] py-8 overflow-hidden border-y border-white/10">
        <div className="flex animate-[marquee_35s_linear_infinite] w-max gap-6">
          {[...googleReviews, ...googleReviews].map((review, i) => (
            <div key={i} className="flex-shrink-0 w-[320px] bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl p-5 flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white font-bold font-headline-md text-[16px]">
                  {review.name[0]}
                </div>
                <div>
                  <p className="text-white font-label-md text-[13px] font-bold">{review.name}</p>
                  <div className="flex items-center gap-0.5 mt-0.5">
                    {[1,2,3,4].map(s => <span key={s} className="text-amber-400 text-[14px]">★</span>)}
                    <span className="text-amber-400 text-[14px]">★</span>
                    <span className="text-white/50 text-[11px] ml-1.5">4.5</span>
                  </div>
                </div>
                <img src="https://www.google.com/favicon.ico" alt="Google" className="w-4 h-4 ml-auto opacity-70" />
              </div>
              <p className="text-white/85 text-[13px] leading-relaxed font-body-md">{review.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="pt-10 md:pt-14 pb-20 md:pb-28 px-gutter bg-[#8C9567]">
        <div
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-12 gap-0 rounded-2xl overflow-hidden shadow-2xl">
            <div className="md:col-span-7 h-80 md:h-[500px] relative">
              <img
                alt="Artisanal Bread"
            ...
            <div className="md:col-span-5 bg-[#F9F6F0] p-8 md:p-12 flex flex-col justify-center">
              <span className="bg-[#8C9567] text-white font-label-md text-[11px] px-4 py-1.5 rounded-full w-max mb-6 uppercase tracking-widest">
                Signature
              </span>
              <h2 className="font-headline-lg text-headline-lg text-[#30111C] mb-4">
                {signatureProduct.title}
              </h2>
              <p className="font-body-md text-body-md text-stone-600 mb-8 leading-relaxed">
                {signatureProduct.description}
              </p>
              <Link
                to="/story"
                className="font-button text-[14px] text-[#8C9567] border-2 border-[#8C9567] rounded-full px-8 py-3 w-max hover:bg-[#8C9567] hover:text-white transition-all duration-300"
              >
                READ MORE
              </Link>
            </div>
